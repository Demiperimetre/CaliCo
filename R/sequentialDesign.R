                       ...
                       public = list(
                         ...
                         initialize = function(md = NA,pr = NA,opt.estim = NA,k = NA)
                         {
                           ...
                           for (i in 1:k)
                           {
                             ...
                             } else
                             {
                               ...
                               self$GP.new  <- km(formula =~1, design=self$doe.new,
                                                  response = z.new,covtype = md$opt.gp$type)
                               self$m       <- c(self$m,min(c(self$m,self$SS(thetaHatNew))))
                             }
                           }
                           cat("\n")
                           cat("======================================\n")
                         ...
                         Crit = function(theta)
                         {
                           ...
                           for (i in 1:nrow(self$md$X))
                           {
                             ...
                             pr   <- predict(self$GP.new,newdata=as.data.frame(t(newD)),type="UK",
                                             interval="confidence",checkNames=FALSE)
                             cov[i]  <- pr$sd
                           }
                           ind <- which(cov==max(cov))
                           print("theta et cov sont affichés")
                           #print(cov)
                           #print(theta)
                           return(c(self$md$X[ind[sample(length(ind))],],theta))
                         },
